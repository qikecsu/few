...
/*	// 百度地图API功能
	...
	*/
	...
	map.centerAndZoom("南京",10);      // 初始化地图,用城市名设置地图中心点
	map.enableScrollWheelZoom(true);     //开启鼠标滚轮缩放
 		...
 		<div dojoType="dijit.layout.StackContainer" id="maincontainer" region="center" style="margin:0; padding:0;overflow:hidden;" > 
		 	...
		 	<div dojoType="dijit.layout.BorderContainer" id="device_list" design="sidebar" title="<b class='tabtitle'>雷达设备列表</b>"  style="background:#180053;padding:0px;" >
				...
				//		alert(item.station_+' '+item.rack_+' '+item.slot_);
 				...
 				<div dojoType="dijit.layout.ContentPane" region="top"  gutters="false" id="query_bar_device"  >
					...
					刷新					
						<script type="dojo/on" data-dojo-event="click" data-dojo-args="evt">
       		 				var store = new dojo.data.ItemFileReadStore({'url':ctxpath+"/DataAction!getDevices.action?cid=${user_cid}"});				
							var grid=dijit.byId("gridDeviceList");
							grid.setStore(store, {id:"*"}, "*");
							grid.startup(); 
				...
				<div dojoType="dijit.layout.ContentPane" splitter="true" region="center" > 
					...
					<table dojoType="dojox.grid.EnhancedGrid" id="gridDeviceList" jsId="gridDeviceList" data-dojo-props="plugins:{indirectSelection: {headerSelector:true, width:'40px', styles:'text-align: center;'}}"  
		 	...
		 	<div dojoType="dijit.layout.BorderContainer" id="realtimemonitoring" gutters="false" title="<b class='tabtitle'>实时监控</b>"  style="padding:0px;" >
				...
				<div dojoType="dijit.layout.ContentPane" region="top"  id="query_bar_event"  >
					...
					//			alert(name+'='+value);
							...
							var store = new dojo.data.ItemFileReadStore({'url':ctxpath+"/DataAction!getDeviceStatus_.action?device="+val+"&cid=${user_cid}"});				
					...
					<div data-dojo-type="dijit.Menu" id="editMenu11" style="display: none;">
						<div data-dojo-type="dijit.MenuItem" data-dojo-props=" iconClass:'dijitIconTask'">火警事件
							<script type="dojo/on" data-dojo-event="click" args="events">
								...
								var store_event = new dojo.data.ItemFileReadStore({'url':ctxpath+"/DataAction!getEvents.action?type=fire&startdate="+startdate+"&enddate="+enddate+"&cid="+companyId});				
						...
						//		alert("操作记录!");
								...
								var store_event = new dojo.data.ItemFileReadStore({'url':ctxpath+"/DataAction!getEvents.action?type=device&startdate="+startdate+"&enddate="+enddate+"&cid="+companyId});				
								var gridevent=dijit.byId("gridEventList");
								gridevent.setStore(store_event, {id:"*"}, "*");
								gridevent.startup(); 
							</script>	
						...
						//		alert("用户信息!");
								...
								var store_event = new dojo.data.ItemFileReadStore({'url':ctxpath+"/DataAction!getEvents.action?type=user&startdate="+startdate+"&enddate="+enddate+"&cid="+companyId});				
								var gridevent=dijit.byId("gridEventList");
								gridevent.setStore(store_event, {id:"*"}, "*");
								gridevent.startup(); 
							</script>	
	 		...
	 		<div dojoType="dijit.layout.BorderContainer" id="allmap" design="sidebar"  style="background:#180053;padding:0px;" >
		 		<div dojoType="dijit.layout.ContentPane" data-dojo-props='title:"Tree"' splitter="true" region="leading" style="width: 180px;"> 
	 				...
	 				<div dojoType="dojo.data.ItemFileReadStore" url="ShenAction!getDevicesfromcompany.action?cid=${user_cid}" jsId="ordStoreDeviceOption_bd"></div>
			     	 ...
			     	 <!--thead定义布局，这是不可缺少的！否则widget将不能工作-->
				    	<thead>
					        <tr>
					           ...
					           <th field="note" editable="true" width="150px">备注</th>
