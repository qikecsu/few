...
//		logger.debug("<<<deleteLines");
	...
	public String getDevices() throws Exception {
		...
		Connection conn = MyDBUtil.getConnection();
		Statement st = conn.createStatement();
		String sql="select device.id, company.name,device.name, device.run, device.fire, device.type, device.deviceid, device.towerid, device._com, device.note, company.id from company, device where device.cid=company.id and company.id='"+cid+"' order by device.id";
		if (Integer.parseInt(cid)<1)
			sql="select device.id, company.name,device.name, device.run, device.fire, device.type, device.deviceid, device.towerid, device._com, device.note, company.id from company, device where device.cid=company.id order by device.id";
		...
		while(rs.next()){
            ...
            String obj="{\"rowId\":"+id+",\"id\":"+id+",\"cid\":"+cid+",\"number\":"+row+",\"company\":\""+company+"\",\"name\":\""+name+"\",\"status\":\""+status+"\",\"fire\":\""+fire+"\",\"type\":\""+type+"\",\"time\":\""+today+"\",\"radar\":\"\", \"camera\":\"\", \"note\":\""+note+"\",\"com\":\""+_com+"\", \"deviceid\":\""+deviceid+"\",\"towerid\":\""+towerid+"\"}";
            if (row>1)
            	obj=",{\"rowId\":"+id+",\"id\":"+id+",\"cid\":"+cid+",\"number\":"+row+",\"company\":\""+company+"\",\"name\":\""+name+"\",\"status\":\""+status+"\",\"fire\":\""+fire+"\",\"type\":\""+type+"\",\"time\":\""+today+"\",\"radar\":\"\", \"camera\":\"\", \"note\":\""+note+"\",\"com\":\""+_com+"\", \"deviceid\":\""+deviceid+"\",\"towerid\":\""+towerid+"\"}";
  ...
  //       ChatAnnotation.broadcast(res);
		...
		if(jsons.size()>0){
			  for(int i=0;i<jsons.size();i++){
			    ...
			    if (fire==1){
			    	String name="宁金5906线182号";
			    	
			    	Statement st = conn.createStatement();
					String sql="select company.name,device.name from company, device where device.cid=company.id and device.id="+job.get("id");									
					ResultSet rs = st.executeQuery(sql);
				    ...
				    String mobile = "13805178732,13182813303,13605175556,13770515415,15050527529";
				    /**************** 发送通知短信 *****************/
			        // 设置您要发送的内容
